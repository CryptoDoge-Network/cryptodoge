...
from cryptodoge.wallet.puzzles.puzzle_utils import (
...
from cryptodoge.wallet.secret_key_store import SecretKeyStore
from cryptodoge.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_cryptodoge(self, cryptodoge_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if cryptodoge_amount < 0:
            utxos = await self.select_coins(abs(cryptodoge_amount), exclude)
        else:
        ...
        cryptodoge_amount = spend_value + cryptodoge_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": cryptodoge_amount}]
