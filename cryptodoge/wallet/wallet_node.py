...
from cryptodoge.consensus.blockchain import ReceiveBlockResult
from cryptodoge.consensus.constants import ConsensusConstants
from cryptodoge.daemon.keychain_proxy import (
...
from cryptodoge.util.chunks import chunks
from cryptodoge.protocols import wallet_protocol
from cryptodoge.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from cryptodoge.protocols.protocol_message_types import ProtocolMessageTypes
from cryptodoge.protocols.wallet_protocol import (
...
from cryptodoge.server.node_discovery import WalletPeers
from cryptodoge.server.outbound_message import Message, NodeType, make_msg
from cryptodoge.server.peer_store_resolver import PeerStoreResolver
...
from cryptodoge.types.weight_proof import WeightProof, SubEpochData
from cryptodoge.util.byte_types import hexstr_to_bytes
from cryptodoge.util.config import WALLET_PEERS_PATH_KEY_DEPRECATED
from cryptodoge.util.default_root import STANDALONE_ROOT_PATH
from cryptodoge.util.ints import uint32, uint64
from cryptodoge.util.keychain import KeyringIsLocked, Keychain
from cryptodoge.util.path import mkdir, path_from_root
from cryptodoge.wallet.util.new_peak_queue import NewPeakQueue, NewPeakQueueTypes, NewPeakItem
from cryptodoge.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
from cryptodoge.wallet.util.wallet_sync_utils import (
...
from cryptodoge.wallet.wallet_coin_record import WalletCoinRecord
from cryptodoge.wallet.wallet_state_manager import WalletStateManager
from cryptodoge.wallet.transaction_record import TransactionRecord
from cryptodoge.wallet.wallet_action import WalletAction
from cryptodoge.util.profiler import profile_task
...
class WalletNode:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'cryptodoge keys' program.")
            return None
    ...
    async def _process_new_subscriptions(self):
        while not self._shut_down:
            ...
            peer: Optional[WSCryptodogeConnection] = None
    ...
    def initialize_wallet_peers(self):
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.cryptodoge.cc"]),
    ...
    def on_disconnect(self, peer: WSCryptodogeConnection):
    ...
    async def long_sync(
        ...
        full_node: WSCryptodogeConnection,
        fork_height: int,
        *,
        rollback: bool,
    ):
    ...
    async def is_peer_synced(
        self, peer: WSCryptodogeConnection, header_block: HeaderBlock, request_time: uint64
    ) -> Optional[uint64]:
        # Get last timestamp
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSCryptodogeConnection) -> None:
    ...
    def get_full_node_peer(self) -> Optional[WSCryptodogeConnection]:
        if self.server is None:
            return None
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        peer: Optional[WSCryptodogeConnection] = self.get_full_node_peer()
        if peer is None:
    ...
    async def new_peak_wallet(self, new_peak: wallet_protocol.NewPeakWallet, peer: WSCryptodogeConnection):
        if self.wallet_state_manager is None:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSCryptodogeConnection) -> int:
        assert self.wallet_state_manager is not None
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSCryptodogeConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
        assert self.wallet_state_manager is not None
        assert self.server is not None
    ...
    async def fetch_puzzle_solution(self, peer: WSCryptodogeConnection, height: uint32, coin: Coin) -> CoinSpend:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], fork_height: Optional[uint32] = None, peer: Optional[WSCryptodogeConnection] = None
    ) -> List[CoinState]:
        assert self.server is not None
        all_nodes = self.server.connection_by_type[NodeType.FULL_NODE]
        if len(all_nodes.keys()) == 0:
    ...
    async def fetch_children(
        self, peer: WSCryptodogeConnection, coin_name: bytes32, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
