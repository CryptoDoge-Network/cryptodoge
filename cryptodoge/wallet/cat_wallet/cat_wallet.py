...
from cryptodoge.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from cryptodoge.wallet.cat_wallet.lineage_store import CATLineageStore
from cryptodoge.wallet.lineage_proof import LineageProof
from cryptodoge.wallet.payment import Payment
from cryptodoge.wallet.puzzles.genesis_checkers import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            cryptodoge_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        cryptodoge_tx = dataclasses.replace(cryptodoge_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(cryptodoge_tx)
    ...
    async def create_tandem_xcd_tx(
        ...
        if fee > amount_to_claim:
            cryptodoge_coins = await self.standard_wallet.select_coins(fee)
            origin_id = list(cryptodoge_coins)[0].name()
            cryptodoge_tx = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
                (await self.standard_wallet.get_new_puzzlehash()),
                fee=uint64(fee - amount_to_claim),
                coins=cryptodoge_coins,
                origin_id=origin_id,  # We specify this so that we know the coin that is making the announcement
                negative_change_allowed=False,
                coin_announcements_to_consume={announcement_to_assert} if announcement_to_assert is not None else None,
            )
            assert cryptodoge_tx.spend_bundle is not None

            message = None
            for spend in cryptodoge_tx.spend_bundle.coin_spends:
            ...
            cryptodoge_coins = await self.standard_wallet.select_coins(fee)
            selected_amount = sum([c.amount for c in cryptodoge_coins])
            cryptodoge_tx = await self.standard_wallet.generate_signed_transaction(
        ...
        return cryptodoge_tx, announcement
    ...
    async def generate_unsigned_spendbundle(
        ...
        regular_cryptodoge_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_cryptodoge_to_claim = payment_amount

        need_cryptodoge_transaction = (fee > 0 or regular_cryptodoge_to_claim > 0) and (fee - regular_cryptodoge_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_cryptodoge_transaction:
                    if fee > regular_cryptodoge_to_claim:
                        cryptodoge_tx, _ = await self.create_tandem_xcd_tx(
                            fee, uint64(regular_cryptodoge_to_claim), announcement_to_assert=announcement
                        )
                    ...
                    elif regular_cryptodoge_to_claim > fee:
                        cryptodoge_tx, _ = await self.create_tandem_xcd_tx(fee, uint64(regular_cryptodoge_to_claim))
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        cryptodoge_spend_bundle = SpendBundle([], G2Element())
        if cryptodoge_tx is not None and cryptodoge_tx.spend_bundle is not None:
            cryptodoge_spend_bundle = cryptodoge_tx.spend_bundle

        return (
            SpendBundle.aggregate(
                [
                    cat_spend_bundle,
                    cryptodoge_spend_bundle,
                ]
            ),
            cryptodoge_tx,
        )
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, cryptodoge_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
        ...
        if cryptodoge_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=cryptodoge_tx.confirmed_at_height,
                    created_at_time=cryptodoge_tx.created_at_time,
                    to_puzzle_hash=cryptodoge_tx.to_puzzle_hash,
                    amount=cryptodoge_tx.amount,
                    fee_amount=cryptodoge_tx.fee_amount,
                    confirmed=cryptodoge_tx.confirmed,
                    sent=cryptodoge_tx.sent,
                    spend_bundle=None,
                    additions=cryptodoge_tx.additions,
                    removals=cryptodoge_tx.removals,
                    wallet_id=cryptodoge_tx.wallet_id,
                    sent_to=cryptodoge_tx.sent_to,
                    trade_id=cryptodoge_tx.trade_id,
                    type=cryptodoge_tx.type,
                    name=cryptodoge_tx.name,
                    memos=[],
                )
            )
