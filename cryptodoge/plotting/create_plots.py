...
from cryptodoge.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from cryptodoge.plotting.util import add_plot_directory
from cryptodoge.plotting.util import stream_plot_info_ph, stream_plot_info_pk
...
from cryptodoge.util.config import config_path_for_filename, load_config
from cryptodoge.util.keychain import Keychain
from cryptodoge.util.path import mkdir
from cryptodoge.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
class PlotKeysResolver:
    ...
    async def get_farmer_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'cryptodoge keys add', 'cryptodoge keys generate' or provide a public key with -f"
            )
    ...
    async def get_pool_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'cryptodoge keys add', 'cryptodoge keys generate' or provide a public key with -p"
            )
