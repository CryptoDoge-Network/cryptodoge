...
from cryptodoge.protocols.protocol_state_machine import message_requires_reply
from cryptodoge.protocols.protocol_timing import API_EXCEPTION_BAN_SECONDS, INVALID_PROTOCOL_BAN_SECONDS
from cryptodoge.protocols.shared_protocol import protocol_version
from cryptodoge.server.introducer_peers import IntroducerPeers
from cryptodoge.server.outbound_message import Message, NodeType
from cryptodoge.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from cryptodoge.util.errors import Err, ProtocolError
from cryptodoge.util.ints import uint16
from cryptodoge.util.network import is_in_network, is_localhost
from cryptodoge.util.ssl_check import verify_ssl_certs_and_keys
...
class CryptodogeServer:
    def __init__(
        ...
        cryptodoge_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        ...
        self.cryptodoge_ca_crt_path, self.cryptodoge_ca_key_path = cryptodoge_ca_crt_key
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.cryptodoge_ca_crt_path, self.cryptodoge_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.cryptodoge_ca_crt_path, self.cryptodoge_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSCryptodogeConnection] = None
        try:
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use cryptodoge's service first.
        try:
