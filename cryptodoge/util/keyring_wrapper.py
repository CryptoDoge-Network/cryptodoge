...
from cryptodoge.util.default_root import DEFAULT_KEYS_ROOT_PATH
from cryptodoge.util.file_keyring import FileKeyring
from cryptodoge.util.misc import prompt_yes_no
...
MAC_KEYCHAIN_MASTER_PASSPHRASE_SERVICE = "Cryptodoge Passphrase"
MAC_KEYCHAIN_MASTER_PASSPHRASE_USER = "Cryptodoge Passphrase"


def check_macos_keychain_keys_present(mac_keychain: MacKeyring) -> bool:
    from keyring.credentials import SimpleCredential
    from cryptodoge.util.keychain import default_keychain_user, default_keychain_service, get_private_key_user, MAX_KEYS
...
class KeyringWrapper:
    ...
    def _get_initial_cached_passphrase(self) -> str:
        from cryptodoge.util.keychain import supports_os_passphrase_storage
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            print(
                "Would you like to set a master passphrase now? Use 'cryptodoge passphrase set' to change the passphrase.\n"
            )

            response = prompt_yes_no("Set keyring master passphrase? (y/n) ")
            if response:
                from cryptodoge.cmds.passphrase_funcs import prompt_for_new_passphrase
                ...
                print(
                    "Will skip setting a master passphrase. Use 'cryptodoge passphrase set' to set the master passphrase.\n"
                )
        else:
            import colorama
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from cryptodoge.util.keychain import get_private_key_user, Keychain, MAX_KEYS
    ...
    def migrate_legacy_keyring_interactive(self):
        ...
        """
        from cryptodoge.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running
