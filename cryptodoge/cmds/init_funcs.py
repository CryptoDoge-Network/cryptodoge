...
from cryptodoge.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_cryptodoge_ca_crt_key,
    make_ca_cert,
...
from cryptodoge.util.ssl_check import (
...
from cryptodoge.cmds.configure import configure
    ...
    with get_config_lock(new_root, "config.yaml"):
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("xcd_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("xcd_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("xcd_target_address") or all_targets[-2] == config["pool"].get(
                    "xcd_target_address"
                ):
        ...
        if "xcd_target_address" not in config["farmer"]:
            print(
                f"Setting the xcd destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
...
def create_all_ssl(root_path: Path):
    ...
    write_ssl_cert_and_key(cryptodoge_ca_crt_path, cryptodoge_ca_crt, cryptodoge_ca_key_path, cryptodoge_ca_key)
...
def init(
    ...
    if create_certs is not None:
        ...
        return cryptodoge_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
