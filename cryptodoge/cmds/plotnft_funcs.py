...
from cryptodoge.cmds.wallet_funcs import print_balance, wallet_coin_unit
from cryptodoge.pools.pool_config import load_pool_config, PoolWalletConfig, update_pool_config
from cryptodoge.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from cryptodoge.protocols.pool_protocol import POOL_PROTOCOL_VERSION
...
from cryptodoge.util.bech32m import encode_puzzle_hash, decode_puzzle_hash
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojos = uint64(int(fee * units["cryptodoge"]))
        ...
        try:
            ...
            while time.time() - start < 10:
                ...
                if len(tx.sent_to) > 0:
                    ...
                    print(f"Do cryptodoge wallet get_transaction -f {fingerprint} -tx 0x{tx_record.name} to get status")
                    return None
        except Exception as e:
            print(f"Error creating plot NFT: {e}\n    Please start both farmer and wallet with:  cryptodoge start -r farmer")
        return
    print("Aborting.")
