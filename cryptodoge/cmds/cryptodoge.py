...
from cryptodoge.cmds.configure import configure_cmd
from cryptodoge.cmds.farm import farm_cmd
from cryptodoge.cmds.init import init_cmd
from cryptodoge.cmds.keys import keys_cmd
from cryptodoge.cmds.netspace import netspace_cmd
from cryptodoge.cmds.passphrase import passphrase_cmd
from cryptodoge.cmds.plots import plots_cmd
from cryptodoge.cmds.show import show_cmd
from cryptodoge.cmds.start import start_cmd
from cryptodoge.cmds.stop import stop_cmd
from cryptodoge.cmds.wallet import wallet_cmd
from cryptodoge.cmds.plotnft import plotnft_cmd
from cryptodoge.cmds.plotters import plotters_cmd
from cryptodoge.cmds.db import db_cmd
from cryptodoge.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
...
from cryptodoge.util.ssl_check import check_ssl
...
@click.group(
    help=f"\n  Manage cryptodoge blockchain infrastructure ({__version__})\n",
    epilog="Try 'cryptodoge start node', 'cryptodoge netspace -d 192', or 'cryptodoge show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from cryptodoge.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
@cli.command("version", short_help="Show cryptodoge version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs cryptodoge daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by cryptodoge start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from cryptodoge.daemon.server import async_run_daemon
