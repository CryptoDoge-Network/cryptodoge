...
from cryptodoge.daemon.client import acquire_connection_to_daemon
from cryptodoge.util.keychain import Keychain, obtain_current_passphrase, supports_os_passphrase_storage
from cryptodoge.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
from cryptodoge.util.misc import prompt_yes_no
from cryptodoge.util.ws_message import WsRpcMessage
...
def read_passphrase_from_file(passphrase_file: TextIOWrapper) -> str:
    passphrase = passphrase_file.read().rstrip(os.environ.get("CRYPTODOGE_PASSPHRASE_STRIP_TRAILING_CHARS", "\r\n"))
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'cryptodoge passphrase set' or 'cryptodoge passphrase remove' to update or remove your passphrase")
        sys.exit(1)
