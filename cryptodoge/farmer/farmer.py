...
from cryptodoge.pools.pool_config import PoolWalletConfig, load_pool_config, add_auth_key
from cryptodoge.protocols import farmer_protocol, harvester_protocol
from cryptodoge.protocols.pool_protocol import (
    ErrorResponse,
...
from cryptodoge.server.server import ssl_context_for_root
...
from cryptodoge.util.bech32m import decode_puzzle_hash
from cryptodoge.util.byte_types import hexstr_to_bytes
from cryptodoge.util.config import load_config, save_config, config_path_for_filename, get_config_lock
from cryptodoge.util.hash import std_hash
from cryptodoge.util.ints import uint8, uint16, uint32, uint64
...
from cryptodoge.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'cryptodoge keys generate' or open the UI."
        ...
        if "xcd_target_address" not in self.config:
            self.config = config["farmer"]
        if "xcd_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "xcd_target_address" not in self.config or "xcd_target_address" not in self.pool_config:
            log.debug("xcd_target_address missing in the config")
            return False
