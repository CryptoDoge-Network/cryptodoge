...
from cryptodoge.cmds.init_funcs import check_keys, cryptodoge_init
from cryptodoge.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from cryptodoge.daemon.keychain_server import KeychainServer, keychain_commands
from cryptodoge.daemon.windows_signal import kill
from cryptodoge.plotters.plotters import get_available_plotters
from cryptodoge.plotting.util import add_plot_directory
from cryptodoge.server.server import ssl_context_for_root, ssl_context_for_server
from cryptodoge.ssl.create_ssl import get_mozilla_ca_crt
from cryptodoge.util.cryptodoge_logging import initialize_logging
from cryptodoge.util.config import load_config
from cryptodoge.util.json_util import dict_to_json_str
from cryptodoge.util.keychain import (
    Keychain,
...
from cryptodoge.util.service_groups import validate_service
from cryptodoge.util.setproctitle import setproctitle
from cryptodoge.util.ws_message import WsRpcMessage, create_payload, format_response
...
service_plotter = "cryptodoge_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        ...
        "cryptodoge_wallet": "start_wallet",
        "cryptodoge_full_node": "start_full_node",
        "cryptodoge_harvester": "start_harvester",
        "cryptodoge_farmer": "start_farmer",
        "cryptodoge_introducer": "start_introducer",
        "cryptodoge_timelord": "start_timelord",
        "cryptodoge_timelord_launcher": "timelord_launcher",
        "cryptodoge_full_node_simulator": "start_simulator",
        "cryptodoge_seeder": "start_seeder",
        "cryptodoge_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Cryptodoge will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
        ...
        try:
            if Keychain.master_passphrase_is_valid(key, force_reload=True):
                ...
                try:
                    ...
                    self.log.exception("Failed to migrate keys silently. Run `cryptodoge keys migrate` manually.")
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["cryptodoge", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("cryptodoge daemon exiting in 5 seconds")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    if service_command == "cryptodoge_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in cryptodoge_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    cryptodoge_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("cryptodoge_daemon")
