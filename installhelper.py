...
# Gets the version from the result of "cryptodoge version"
...
def get_cryptodoge_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["cryptodoge", "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_cryptodoge_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/cryptodoge-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/cryptodoge-gui/packages/gui/package.json")
