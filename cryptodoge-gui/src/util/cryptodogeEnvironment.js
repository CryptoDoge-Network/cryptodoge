...
const PY_FOLDER = '../cryptodoge/daemon';
...
const getCryptodogeVersion = () => {
  let version = null;
  const exePath = getExecutablePath('cryptodoge');
  // first see if we can get a cryptodoge exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or cryptodoge is on the path
    try {
      version = child_process
