...
// import { Flex, FormatLargeNumber } from '@cryptodoge/core';
...
function prepareGraphPoints(
  ...
  */
  ...
  const points = [
    {
      x: peak.height,
      y: Math.max(0, mojo_to_cryptodoge(start)),
      tooltip: mojo_to_cryptodoge(balance),
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: Math.max(0, mojo_to_cryptodoge(start)),
      tooltip: mojo_to_cryptodoge(start),
    });
  });
