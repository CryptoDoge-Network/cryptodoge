...
import { AlertDialog } from '@cryptodoge/core';
import {
  Typography,
  Button,
  Box,
...
import { cryptodoge_to_mojo } from '../../../util/cryptodoge';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      cryptodogeper_input.value === '' ||
      Number(cryptodogeper_input.value) === 0 ||
      !Number(cryptodogeper_input.value) ||
      isNaN(Number(cryptodogeper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const cryptodogeper = cryptodoge_to_mojo(cryptodogeper_input.value);
    const cryptodogeper_value = Number.parseInt(Number(cryptodogeper));
    const userpubkey = userpubkey_input.value;
    const amount = cryptodoge_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = cryptodoge_to_mojo(fee_input.value);
