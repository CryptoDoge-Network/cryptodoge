...
import { mojo_to_cryptodoge } from '../../util/cryptodoge';
import {
  calculatePoolReward,
...
export default function Block() {
  ...
  const poolReward = mojo_to_cryptodoge(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_cryptodoge(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const cryptodogeFees = blockRecord.fees
    ? mojo_to_cryptodoge(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.cryptodogeexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.cryptodogeexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: cryptodogeFees ? `${cryptodogeFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Cryptodoge
            </Trans>
          </BlockTitle>
        }
        action={
