...
import cryptodogeEnvironment from '../util/cryptodogeEnvironment';
import cryptodogeConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!cryptodogeEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    cryptodogeConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!cryptodogeConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !cryptodogeConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (cryptodogeConfig.manageDaemonLifetime()) {
        cryptodogeEnvironment.startCryptodogeDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Cryptodoge Wiki' }),
            click: () => {
              openExternal(
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Cryptodoge-Network/cryptodoge/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Cryptodoge-Network/cryptodoge/issues',
              );
            },
          },
          {
    ...
    if (process.platform === 'darwin') {
      ...
      template.unshift({
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Cryptodoge' }),
            click: () => {
              openAbout();
            },
          },
