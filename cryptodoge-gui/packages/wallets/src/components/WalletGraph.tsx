...
import { TransactionType } from '@cryptodoge/api';
import type { Transaction } from '@cryptodoge/api';
import { mojoToCryptodoge, blockHeightToTimestamp } from '@cryptodoge/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: Math.max(0, Number(mojoToCryptodoge(start))),
      tooltip: mojoToCryptodoge(balance),
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: Math.max(0, Number(mojoToCryptodoge(start))),
      tooltip: mojoToCryptodoge(start),
    });
  });
