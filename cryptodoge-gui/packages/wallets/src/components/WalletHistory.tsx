...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToCryptodogeLocaleString, mojoToCATLocaleString } from '@cryptodoge/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@cryptodoge/api-react';
import styled from 'styled-components';
import type { Row } from '@cryptodoge/core';
import { WalletType, TransactionType } from '@cryptodoge/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToCryptodogeLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToCryptodogeLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
