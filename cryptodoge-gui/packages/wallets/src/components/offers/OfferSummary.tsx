...
import { type OfferSummaryRecord  } from '@cryptodoge/api';
import {
  Flex,
  StateColor,
  mojoToCryptodoge,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries[0][0].toLowerCase() === 'xcd' ? Number(mojoToCryptodoge(makerEntries[0][1])) : Number(mojoToCAT(makerEntries[0][1]));
  const takerAmount = takerEntries[0][0].toLowerCase() === 'xcd' ? Number(mojoToCryptodoge(takerEntries[0][1])) : Number(mojoToCAT(takerEntries[0][1]));
