...
import { OfferTradeRecord } from '@cryptodoge/api';
import fs from 'fs';
...
import { Trade as TradeIcon } from '@cryptodoge/icons';
import { useCancelOfferMutation, useGetOfferDataMutation, useGetWalletsQuery } from '@cryptodoge/api-react';
...
function ConfirmOfferCancellation(props: ConfirmOfferCancellationProps) {
  ...
  async function handleConfirm() {
    const { fee: xcdFee } = methods.getValues();
    var fee = 0;

    if (cancelWithTransaction) {
      fee = Number.parseFloat(cryptodogeToMojo(xcdFee));
    }
      ...
      <Grid container>
        <Grid xs={12} md={6} lg={5} item>
          <CardHero>
            ...
            <Typography variant="body1">
              <Trans>
                Create an offer to exchange XCD or other tokens. View an offer to inspect and accept an offer made by another party.
              </Trans>
            </Typography>
