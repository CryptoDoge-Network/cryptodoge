...
import {
  ...
  useOpenExternal,
  cryptodogeToMojo,
  mojoToCryptodogeLocaleString,
  mojoToCATLocaleString,
} from '@cryptodoge/core';
import {
  Box,
  Button,
  Divider,
...
import { OfferSummaryRecord, OfferTradeRecord, OfferCoinOfInterest, WalletType } from '@cryptodoge/api';
import { useCheckOfferValidityMutation, useTakeOfferMutation } from '@cryptodoge/api-react';
import {
  colorForOfferState,
...
function OfferDetails(props: OfferDetailsProps) {
      ...
      field: (coin: OfferCoinOfInterest) => {
        return (
          <Typography variant="body2">
            <Flex flexDirection="row" flexGrow={1} gap={1}>
              {mojoToCryptodogeLocaleString(coin.amount)}
            </Flex>
          </Typography>
        )
      },
    ...
    },
    {
      field: (coin: OfferCoinOfInterest) => {
        return (
          ...
          >
            <Link
              onClick={(event: React.SyntheticEvent) => handleLinkClicked(event, `https://alltheblocks.net/cryptodoge/blockchain/coin/${coin.parentCoinInfo}`)}
            >
              {coin.parentCoinInfo}
            </Link>
          </Tooltip>
    ...
    },
    {
      field: (coin: OfferCoinOfInterest) => {
        return (
          ...
          >
            <Link
              onClick={(event: React.SyntheticEvent) => handleLinkClicked(event, `https://alltheblocks.net/cryptodoge/blockchain/puzzlehash/${coin.puzzleHash}`)}
            >
              {coin.puzzleHash}
            </Link>
          </Tooltip>
  ...
  async function handleAcceptOffer(formData: any) {
    const { fee } = formData;
    const feeInMojos = fee ? Number.parseFloat(cryptodogeToMojo(fee)) : 0;
