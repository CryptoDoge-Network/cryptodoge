...
function getConfigRootDir(net = 'mainnet'): string {
  ...
  return 'CRYPTODOGE_ROOT' in process.env
    ? untildify(process.env.CRYPTODOGE_ROOT)
    : path.join(homedir, '.cryptodoge', net);
}
...
export default async function loadConfig(net?: string): Promise<{
  ...
  try {
    ...
    const daemonPort = get(config, 'ui.daemon_port', 57690);
