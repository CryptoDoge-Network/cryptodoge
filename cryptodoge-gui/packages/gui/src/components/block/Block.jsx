...
import { useGetBlockQuery, useGetBlockRecordQuery  } from '@cryptodoge/api-react'
...
export default function Block() {
  ...
  const poolReward = mojoToCryptodoge(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToCryptodoge(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const cryptodogeFees = blockRecord.fees !== undefined
    ? mojoToCryptodoge(blockRecord.fees)
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/cryptodoge/puzzlehash/${blockRecord.farmerPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://alltheblocks.net/cryptodoge/puzzlehash/${blockRecord.poolPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
