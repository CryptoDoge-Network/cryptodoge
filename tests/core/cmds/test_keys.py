...
from cryptodoge.cmds.cryptodoge import cli
from cryptodoge.cmds.keys import delete_all_cmd, generate_and_print_cmd, show_cmd, sign_cmd, verify_cmd
from cryptodoge.util.config import load_config
from cryptodoge.util.file_keyring import FileKeyring
from cryptodoge.util.keychain import DEFAULT_USER, DEFAULT_SERVICE, Keychain, generate_mnemonic
from cryptodoge.util.keyring_wrapper import DEFAULT_KEYS_ROOT_PATH, KeyringWrapper, LegacyKeyring
...
def empty_keyring():
    with TempKeyring(user="user-cryptodoge-1.8", service="cryptodoge-user-cryptodoge-1.8") as keychain:
        yield keychain
...
class TestKeysCommands:
    def test_generate_with_new_config(self, tmp_path, empty_keyring):
        ...
        the correct xcd_target_address entries.
        """
        ...
        # Verify that the config has the correct xcd_target_address entries
        address_matches = re.findall(r"xcd1[^\n]+", result.output)
        assert len(address_matches) > 1
        address = address_matches[0]

        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["xcd_target_address"] == address
        assert config["pool"]["xcd_target_address"] == address
    ...
    def test_generate_with_existing_config(self, tmp_path, empty_keyring):
        ...
        the original xcd_target_address entries.
        """
        ...
        assert existing_config["farmer"]["xcd_target_address"] == address
        assert existing_config["pool"]["xcd_target_address"] == address
        ...
        # Verify that the config's xcd_target_address entries have not changed
        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["xcd_target_address"] == existing_config["farmer"]["xcd_target_address"]
        assert config["pool"]["xcd_target_address"] == existing_config["pool"]["xcd_target_address"]

    def test_show(self, keyring_with_one_key):
        """
        Test that the `cryptodoge keys show` command shows the correct key.
        """
    ...
    def test_show_mnemonic(self, keyring_with_one_key):
        """
        Test that the `cryptodoge keys show --show-mnemonic-seed` command shows the key's mnemonic seed.
        """
    ...
    def test_generate_and_print(self):
        """
        Test the `cryptodoge keys generate_and_print` command.
        """
    ...
    def test_sign(self, keyring_with_one_key):
        """
        Test the `cryptodoge keys sign` command.
        """
    ...
    def test_sign_non_observer(self, keyring_with_one_key):
        """
        Test the `cryptodoge keys sign` command with a non-observer key.
        """
    ...
    def test_verify(self):
        """
        Test the `cryptodoge keys verify` command.
        """
    ...
    def test_derive_search(self, tmp_path, keyring_with_one_key):
        """
        Test the `cryptodoge keys derive search` command, searching a public and private key
        """
    ...
    def test_derive_search_wallet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `cryptodoge keys derive search` command, searching for a wallet address
        """
        ...
        result: Result = runner.invoke(
            ...
            [
                ...
                "--search-type",
                "address",
                "xcd1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd",
            ],
        )
        ...
        assert (
            result.output.find(
                (
                    ...
                    "xcd1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd (HD path: m/12381/8444/2/30)"
                )
            )
            != -1
        )
    ...
    def test_derive_search_failure(self, tmp_path, keyring_with_one_key):
        """
        Test the `cryptodoge keys derive search` command with a failing search.
        """
    ...
    def test_derive_search_hd_path(self, tmp_path, empty_keyring, mnemonic_seed_file):
        """
        Test the `cryptodoge keys derive search` command, searching under a provided HD path.
        """
    ...
    def test_derive_wallet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `cryptodoge keys derive wallet-address` command, generating a couple of wallet addresses.
        """
        ...
        assert (
            result.output.find(
                (
                    ...
                    "xcd1jp2u7an0mn9hdlw2x05nmje49gwgzmqyvh0qmh6008yksetuvkfs6wrfdq"
                )
            )
            != -1
        )
        assert (
            result.output.find(
                (
                    ...
                    "xcd1006n6l3x5e8exar8mlj004znjl5pq0tq73h76kz0yergswnjzn8sumvfmt"
                )
            )
            != -1
        )
    ...
    def test_derive_child_keys(self, tmp_path, keyring_with_one_key):
        """
        Test the `cryptodoge keys derive child-keys` command, generating a couple of derived keys.
        """
    ...
    def test_migration_not_needed(self, tmp_path, setup_keyringwrapper, monkeypatch):
        """
        Test the `cryptodoge keys migrate` command when no migration is necessary
        """
    ...
    def test_migration_full(self, tmp_path, setup_legacy_keyringwrapper):
        """
        Test the `cryptodoge keys migrate` command when a full migration is needed
        """
        ...
        current_version_str = pkg_resources.get_distribution("cryptodoge").version
    ...
    def test_migration_incremental(self, tmp_path, keyring_with_one_key, monkeypatch):
        ...
        from cryptodoge.util import keyring_wrapper
    ...
    def test_migration_checked(self, tmp_path, monkeypatch):
        ...
        assert pkg_resources.get_distribution("cryptodoge").version == "1.2.11.dev123"
