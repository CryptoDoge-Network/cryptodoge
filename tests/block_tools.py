...
from cryprotdoge.cmds.init_funcs import create_all_ssl, create_default_cryprotdoge_config
from cryprotdoge.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
...
from cryprotdoge.full_node.generator import setup_generator_args
from cryprotdoge.full_node.mempool_check_conditions import GENERATOR_MOD
from cryprotdoge.plotting.create_plots import create_plots, PlotKeys
from cryprotdoge.consensus.block_creation import unfinished_block_to_full_block
from cryprotdoge.consensus.block_record import BlockRecord
from cryprotdoge.consensus.block_rewards import calculate_base_farmer_reward, calculate_pool_reward
from cryprotdoge.consensus.blockchain_interface import BlockchainInterface
from cryprotdoge.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
from cryprotdoge.consensus.condition_costs import ConditionCost
...
from cryprotdoge.consensus.deficit import calculate_deficit
from cryprotdoge.consensus.full_block_to_block_record import block_to_block_record
from cryprotdoge.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from cryprotdoge.consensus.pot_iterations import (
    calculate_ip_iters,
...
from cryprotdoge.consensus.vdf_info_computation import get_signage_point_vdf_info
from cryprotdoge.full_node.signage_point import SignagePoint
from cryprotdoge.plotting.util import PlotInfo, PlotsRefreshParameter, PlotRefreshResult, parse_plot_info
from cryprotdoge.plotting.manager import PlotManager
from cryprotdoge.server.server import ssl_context_for_server
from cryprotdoge.types.blockchain_format.classgroup import ClassgroupElement
from cryprotdoge.types.blockchain_format.coin import Coin, hash_coin_list
from cryprotdoge.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
from cryprotdoge.types.blockchain_format.pool_target import PoolTarget
from cryprotdoge.types.blockchain_format.program import INFINITE_COST
from cryprotdoge.types.blockchain_format.proof_of_space import ProofOfSpace
from cryprotdoge.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from cryprotdoge.types.blockchain_format.sized_bytes import bytes32
from cryprotdoge.types.blockchain_format.slots import (
...
from cryprotdoge.types.blockchain_format.vdf import VDFInfo, VDFProof
from cryprotdoge.types.end_of_slot_bundle import EndOfSubSlotBundle
from cryprotdoge.types.full_block import FullBlock
from cryprotdoge.types.generator_types import BlockGenerator, CompressorArg
from cryprotdoge.types.spend_bundle import SpendBundle
from cryprotdoge.types.unfinished_block import UnfinishedBlock
from cryprotdoge.util.bech32m import encode_puzzle_hash
from cryprotdoge.util.block_cache import BlockCache
from cryprotdoge.util.condition_tools import ConditionOpcode
from cryprotdoge.util.config import load_config, save_config
from cryprotdoge.util.hash import std_hash
from cryprotdoge.util.ints import uint8, uint16, uint32, uint64, uint128
from cryprotdoge.util.keychain import Keychain, bytes_to_mnemonic
from cryprotdoge.util.merkle_set import MerkleSet
from cryprotdoge.util.prev_transaction_block import get_prev_transaction_block
from cryprotdoge.util.path import mkdir
from cryprotdoge.util.vdf_prover import get_vdf_info_and_proof
from tests.wallet_tools import WalletTool
from cryprotdoge.wallet.derive_keys import (
...
class BlockTools:
    ...
    def __init__(
        ...
        create_default_cryprotdoge_config(root_path)
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="cryprotdoge-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `cryprotdoge generate keys`")
    ...
    async def setup_plots(self):
        ...
        try:
            ...
            plot_keys_2 = PlotKeys(self.farmer_pk, None, encode_puzzle_hash(self.pool_ph, "xcd"))
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("CRYPTODOGE_ROOT", "~/.cryprotdoge/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
