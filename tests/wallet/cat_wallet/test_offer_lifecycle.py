...
from cryptodoge.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
    construct_cat_puzzle,
    SpendableCAT,
    unsigned_spend_bundle_for_spendable_cats,
)
from cryptodoge.wallet.payment import Payment
from cryptodoge.wallet.trading.offer import Offer, NotarizedPayment
...
# This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/XCD coins
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            cryptodoge_coins: List[Coin] = all_coins[None]
            red_coins: List[Coin] = all_coins["red"]
            blue_coins: List[Coin] = all_coins["blue"]

            # Create an XCD Offer for RED
            cryptodoge_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            cryptodoge_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                cryptodoge_requested_payments, cryptodoge_coins
            )
            cryptodoge_announcements: List[Announcement] = Offer.calculate_announcements(cryptodoge_requested_payments)
            cryptodoge_secured_bundle: SpendBundle = generate_secure_bundle(cryptodoge_coins, cryptodoge_announcements, 1000)
            cryptodoge_offer = Offer(cryptodoge_requested_payments, cryptodoge_secured_bundle)
            assert not cryptodoge_offer.is_valid()

            # Create a RED Offer for XCD
            ...
            new_offer = Offer.aggregate([cryptodoge_offer, red_offer])
            ...
            # Create yet another offer of BLUE for XCD and RED
            ...
            assert new_offer.summary() == (
                ...
                {"xcd": 900, str_to_tail_hash("red").hex(): 350},
            )
