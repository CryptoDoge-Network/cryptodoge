...
from cryptodoge.rpc.full_node_rpc_api import FullNodeRpcApi
from cryptodoge.rpc.full_node_rpc_client import FullNodeRpcClient
from cryptodoge.rpc.rpc_server import start_rpc_server
from cryptodoge.rpc.wallet_rpc_api import WalletRpcApi
...
from cryptodoge.util.ints import uint16, uint32, uint64
...
from cryptodoge.wallet.transaction_sorting import SortKey
from cryptodoge.wallet.util.compute_memos import compute_memos
...
class TestWalletRpc:
    ...
    async def test_wallet_rpc(self, two_wallet_nodes, trusted, bt, self_hostname):
        ...
        try:
            ...
            addr = encode_puzzle_hash(await wallet_node_2.wallet_state_manager.main_wallet.get_new_puzzlehash(), "txcd")
            tx_amount = 15600000
            try:
            ...
            for i in range(0, 5):
                await client.farm_block(encode_puzzle_hash(ph_2, "txcd"))
            ...
            # Test RELEVANCE
            await client.send_transaction("1", 1, encode_puzzle_hash(ph_2, "txcd"))  # Create a pending tx
            ...
            await client.send_transaction("1", 1, encode_puzzle_hash(ph_by_addr, "txcd"))
            await client.farm_block(encode_puzzle_hash(ph_by_addr, "txcd"))
            ...
            tx_for_address = await wallet_rpc_api.get_transactions(
                {"wallet_id": "1", "to_address": encode_puzzle_hash(ph_by_addr, "txcd")}
            )
            ...
            # Create an offer of 5 cryptodoge for one CAT
            ...
            assert summary == {"offered": {"xcd": 5}, "requested": {col.hex(): 1}, "fees": 1}
