...
from cryptodoge.protocols.full_node_protocol import RespondBlock
...
from cryptodoge.wallet.derive_keys import find_authentication_sk, find_owner_sk
...
from cryptodoge.util.ints import uint16, uint32
...
class TestPoolWalletRpc:
    ...
    async def test_leave_pool(self, setup, fee, trusted, self_hostname):
        ...
        try:
            ...
            async def have_cryptodoge():
                await farm_blocks(full_node_api, our_ph, 1)
                return (await wallets[0].get_confirmed_balance()) > 0

            await time_out_assert(timeout=WAIT_SECS, function=have_cryptodoge)
