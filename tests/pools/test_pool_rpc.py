...
from cryprotdoge.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from cryprotdoge.protocols import full_node_protocol
from cryprotdoge.protocols.full_node_protocol import RespondBlock
from cryprotdoge.rpc.rpc_server import start_rpc_server
from cryprotdoge.rpc.wallet_rpc_api import WalletRpcApi
from cryprotdoge.rpc.wallet_rpc_client import WalletRpcClient
from cryprotdoge.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
from cryprotdoge.util.ints import uint16, uint32
from cryprotdoge.wallet.derive_keys import master_sk_to_local_sk
from cryprotdoge.wallet.transaction_record import TransactionRecord
from cryprotdoge.wallet.util.wallet_types import WalletType
...
class TestPoolWalletRpc:
    ...
    async def create_pool_plot(self, p2_singleton_puzzle_hash: bytes32) -> bytes32:
        ...
        try:
            plot_keys = PlotKeys(bt.farmer_pk, None, encode_puzzle_hash(p2_singleton_puzzle_hash, "txcd"))
    ...
    async def test_leave_pool(self, setup):
        ...
        try:
            ...
            async def have_cryprotdoge():
                await self.farm_blocks(full_node_api, our_ph, 1)
                return (await wallets[0].get_confirmed_balance()) > 0

            await time_out_assert(timeout=WAIT_SECS, function=have_cryprotdoge)
